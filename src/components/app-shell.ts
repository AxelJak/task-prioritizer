...
export class AppShell extends LitElement {
  ...
  @state() private notification: { message: string; type: 'success' | 'error' | 'info' } | null = null;
  ...
  static styles = css`
    ...
    .status-indicator.ai-enabled { background: #17a2b8; }
    .status-indicator.ai-disabled { background: #ffc107; }
  ...
  private setupEventListeners() {
    ...
    window.addEventListener('ai-analysis-complete', (e: any) => {
      ...
      const updatedTasks = AIAnalyzer.mapResponseToTasks(analyzedTasks, analysis, provider);
    ...
    this.addEventListener('tasks-added', (e: any) => {
      ...
      this.showNotification(`Added ${e.detail.tasks.length} task${e.detail.tasks.length === 1 ? '' : 's'}`, 'success');
    });
  ...
  private async updateTasksWithAIAnalysis(analyzedTasks: Task[]) {
    try {
      ...
      this.showNotification(`AI analysis completed for ${analyzedTasks.length} task${analyzedTasks.length === 1 ? '' : 's'}`, 'success');
    } catch (error) {
    ...
    return html`
      <div class="container">
        <header class="header">
          ...
          <p class="subtitle">Categorize and prioritize your tasks with AI analysis</p>
          
          <div class="status-bar">
            <div class="status-item">
              <div class="status-indicator ${this.isOnline ? 'online' : 'offline'}"></div>
              ${this.isOnline ? 'Online' : 'Offline'}
            </div>
            <div class="status-item">
              <div class="status-indicator ${aiAnalyzer.isConfigured() ? 'ai-enabled' : 'ai-disabled'}"></div>
              AI ${aiAnalyzer.isConfigured() ? 'Enabled' : 'Disabled'}
            </div>
            <div class="status-item">
              ${this.tasks.length} task${this.tasks.length === 1 ? '' : 's'} total
            </div>
          </div>
          
          <button class="settings-btn" @click=${this.toggleSettings} title="Settings">
            ⚙️
          </button>
        </header>
        ...
        ${this.showSettings ? html`
          <div class="settings-modal" @click=${(e: Event) => {
            ...
            <div class="settings-content">
              ...
              <div class="form-group">
                ...
                <div class="form-help">
                  Choose your AI provider. Changing provider will clear the current API key.
                </div>
              </div>
              ...
              ${this.provider ? html`
                <div class="form-group">
                  ...
                  <input 
                    ...
                    placeholder=${this.provider === 'anthropic' ? 'Enter your Anthropic API key' : 
                                 this.provider === 'openai' ? 'Enter your OpenAI API key' : 
                                 'Enter your Google AI API key'}
                  />
                  <div class="form-help">
                    ${this.provider === 'anthropic' ? 
                      html`Get your API key from <a href="https://console.anthropic.com/" target="_blank">console.anthropic.com</a>` :
                      this.provider === 'openai' ?
                      html`Get your API key from <a href="https://platform.openai.com/api-keys" target="_blank">platform.openai.com</a>` :
                      html`Get your API key from <a href="https://aistudio.google.com/app/apikey" target="_blank">Google AI Studio</a>`
                    }
                  </div>
                </div>
              ` : ''}
              ...
              <div class="settings-actions">
                <button class="btn btn-secondary" @click=${() => this.showSettings = false}>
                  Cancel
                </button>
                <button 
